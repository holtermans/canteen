...
public class WxAuthController {
    ...
    public Object loginByWeixin(@RequestBody WxLoginInfo wxLoginInfo, HttpServletRequest request) {
        ...
        if (user == null) {//没有找到，证明是我们这里的新用户
            ...
            userService.add(user);  //添加新用户

            // 新用户发送注册优惠券
            couponAssignService.assignForRegister(user.getId());
        } else {  //微信已经登录，但是没有认证需要认证为我们单位的员工
            user.setLastLoginTime(LocalDateTime.now());
            user.setLastLoginIp(IpUtil.getIpAddr(request));
            user.setSessionKey(sessionKey);
            if (userService.updateById(user) == 0) {
    ...
    @PostMapping("login_fs")
    public Object loginByWxFs(@RequestBody WxLoginInfo wxLoginInfo, HttpServletRequest request) {
        String code = wxLoginInfo.getCode();
        UserInfo userInfo = wxLoginInfo.getUserInfo();
        if (code == null || userInfo == null) {
            return ResponseUtil.badArgument();
        ...
        } else if (user.getBcUserId() == null && user != null) {  //非新用户，微信已经登录，但是没有认证需要认证为我们单位的员工
            //返回用户信息，不返回token
            user.setLastLoginTime(LocalDateTime.now());
            user.setLastLoginIp(IpUtil.getIpAddr(request));
            user.setSessionKey(sessionKey);
    ...
    @PostMapping("login_sec")
    public Object loginByWxSec(@RequestBody String body, HttpServletRequest request) {
        String username = JacksonUtil.parseString(body, "username");
        String mobile = JacksonUtil.parseString(body, "mobile");
        String authCode = JacksonUtil.parseString(body, "authCode");
        String wxCode = JacksonUtil.parseString(body, "wxCode");
        Config config = configService.queryByName("authCode");
        String configAuthCode = config.getValue();  //配置的授权码

        System.out.println(authCode);
        System.out.println(configAuthCode);
        String sessionKey = null;
        String openId = null;

        try {
            WxMaJscode2SessionResult result = this.wxService.getUserService().getSessionInfo(wxCode); //这个是根据临时登录凭证获取 用户唯一标识 openId 和 本次登录会话秘钥 sessionKey的
            sessionKey = result.getSessionKey();
            openId = result.getOpenid();
        } catch (Exception e) {
            e.printStackTrace();
        ...
        if (user.getBcUserId() == null || bcUserService.findById(user.getBcUserId()) == null) { //如果是未关联的
            ...
            user.setBcUserId(id);
            userService.updateById(user);//更新关联用户
        }
// userInfo
        UserInfo userInfo = new UserInfo();
    ...
    public Object registerCaptcha(@RequestBody String body) {
        ...
        String code = CharUtil.getRandomNum(6);//获取6位随机验证码
